...
locals {
  ...
  audit_includes_names  = "${length(var.audit_includes) > 0 ? split(",", replace(upper(join(",", var.audit_includes)), "/[\\.-_]/", "")) : list()}"
  audit_includes        = "${indent(6, join("\n", formatlist("ES_SECURITY_AUDIT_INCLUDES_%s: %s", local.audit_includes_names, var.audit_includes)))}"
  audit_excludes_names  = "${length(var.audit_excludes) > 0 ? split(",", replace(upper(join(",", var.audit_excludes)), "/[\\.-_]/", "")) : list()}"
